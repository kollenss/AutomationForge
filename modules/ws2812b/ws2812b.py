    ...
    return [{
        ...
        'params': [
            {'key': 'name',          'label': 'Zone Name',     'type': 'text',   'default': 'LEDs',
             'description': 'Display name shown on the card. E.g. LEDs.'},
            {'key': 'leds',          'label': 'LEDs in Zone',  'type': 'text',   'default': '1-2',
             'description': 'Which LEDs in the chain belong to this zone (1-based). E.g. 1-5 for a range, 1,5 for individual LEDs, or 1-3,5 combined.'},
            {'key': 'default_color', 'label': 'Default Color', 'type': 'text',   'default': 'white',
             'description': 'Colour used when an effect does not specify one. Name (red, blue, white, …) or hex like #FF8800.'},
            {'key': 'brightness',    'label': 'Brightness',    'type': 'number', 'default': 128,
             'description': 'Master brightness for this zone, 0 (off) to 255 (full). E.g. 128.'},
        ],
        'inputs': [
            {'key': 'set_color', 'label': 'Set Color',
             'description': 'Static colour. Value sets the colour — e.g. red, blue, white, or hex #FF8800. Overrides Default Color.'},
            {'key': 'blink',     'label': 'Blink',
             'description': 'Blink then restore. Numeric value = blink count (e.g. 3); colour string = blink colour (e.g. red). Done fires when complete.'},
            {'key': 'pulse',     'label': 'Pulse',
             'description': 'Breathing animation. Colour string sets the colour (e.g. blue). Runs until Off. Done fires immediately on start.'},
            {'key': 'chase',     'label': 'Chase',
             'description': 'A single lit pixel sweeps back and forth through the zone (scanner). Colour string sets the colour (e.g. green). Runs until Off.'},
            {'key': 'fill',      'label': 'Fill',
             'description': 'Light the zone one LED at a time in order; each stays on. Colour string sets the colour. Done fires when the last LED is lit.'},
